...
class XGB_Node:
    ...
    def set_estimate_fcns(self, fcn_estimate):
        self.fcn_estimate = fcn_estimate
        self.output = optimal_output(self.loss_fcn, self.regularization_param, self.y, self.fcn_estimate(self.X))
        self.similarity_score = similarity_score(self.loss_fcn, self.regularization_param, self.y, self.fcn_estimate(self.X))
    ...
    def create_child(self, generate: bool = False, max_depth: float = float('inf'), min_points: int = 1):
        def feature_best_split(X, y, X_all):
            ...
            for i in range(1, len(X)):
                ...
                if ss > best_ss: # Getting the best split that minimizes the SSE for that particular feature
        ...
        def best_split(X: pd.DataFrame, y: pd.Series):
            ...
            for feature in X.columns:
                ...
                if ss > best_ss: # Getting the best split among all features that minimizes the SSE
